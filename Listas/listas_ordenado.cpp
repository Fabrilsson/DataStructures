...
//mantém a lista ordenada em ordem alfabética crescente
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <conio.h>
...
void alocaOrdenado(LISTA *lista, DADOS info){
    NODO *p,*q,*n;//n é o ponteiro para alocar o novo lista
    int incluiu=0;
    ...
    if (lista->inicio!=NULL){
        p=lista->inicio;//p é o ponteiro para navegação na lista
        if (strcmp(info.nome,p->nome)<0){ //verifica se o novo elemento é menor que o primeiro da lista
           n->proximo=lista->inicio;
           lista->inicio=n; 
           lista->qtd=lista->qtd+1; 
        }
        else{
        q=p->proximo;//q é o ponteiro do próximo
        while((q!=NULL)&&(incluiu==0)){
             if (strcmp(info.nome,q->nome)<0){//elemento novo fica entre ponteiro p e q
        ...
        if (incluiu == 0){//indica que novo nodo será incluido no fim
            p->proximo=n;  
            lista->fim=n;
            lista->qtd=lista->qtd+1;           
        }
    ...
    else{//inclui primeiro nodo da lista
      lista->inicio=n;
      lista->fim=n;
      lista->qtd=1;
    }
...
void exclui(LISTA *lista, DADOS info){
   ...
   int sai=0;
   p=lista->inicio;
   if (lista->inicio!=NULL)//lista não está vazia
    if(lista->inicio==lista->fim){//lista tem somente um elemento
      free(p);
      lista->inicio=NULL;
      lista->fim=NULL;
      lista->qtd=0;    
    }
    else //inicio é diferente de fim
        ...
        //removendo primeiro elemento da lista
           ...
           free(p);            
             ...
             if ((q->codigo==info.codigo)||(strcmp(q->nome,info.nome)==0)){
               //será excluido o nodo q
               p->proximo=q->proximo;//p que aponta para q passa a apontar para quem q aponta
               if (lista->fim==q)
                  lista->fim=p;//atualiza fim da lista se q era o último elemento
               free(q);
               sai=1;
               lista->qtd=lista->qtd-1;
             }
